...
public class SlideEffectDemo extends Application {
    ...
    private void configureBox(final VBox root) {
        ...
        // BOTTOM PANE
        final Stop[] stops = new Stop[] { new Stop(0, Color.web("#F89C8C")), new Stop(1, Color.web("#BE250A")) };
        final LinearGradient lg = new LinearGradient(0, 0, 0, 1, true, CycleMethod.NO_CYCLE, stops);
        bottomPane = new StackPane();
        bottomPane.getChildren().addAll(RectangleBuilder.create().width(boxBounds.getWidth()).height(boxBounds.getHeight()).fill(lg).build(), TextBuilder
                .create().text("Click the above \"Slide Down\" button to see the top pane content...").wrappingWidth(200).font(Font.font("Arial", 22)).build());

        // TOP PANE
        final Stop[] stops2 = new Stop[] { new Stop(0, Color.web("#FFFFFF")), new Stop(1, Color.web("#50AABC")) };
        final LinearGradient lg2 = new LinearGradient(0, 0, 0, 1, true, CycleMethod.NO_CYCLE, stops2);
        final StackPane sp1 = new StackPane();
        sp1.getChildren().add(TextBuilder.create().text("Click the below \"Slide Up\" button to see the bottom pane content...").wrappingWidth(200)
                .font(Font.font("Arial", 22)).build());
    ...
    private void setAnimation() {
        ...
        final EventHandler<ActionEvent> onFinished = event -> timelineBounce.play();
    ...
    private HBox getActionPane() {
        ...
        hb.setSpacing(10);
        hb.setPrefHeight(40);
        final Button upBtn = new Button("Slide Up");
        upBtn.setOnAction(event -> timelineUp.play());
        final Button downBtn = new Button("Slide Down");
        downBtn.setOnAction(event -> timelineDown.play());
